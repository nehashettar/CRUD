...
public class Student_DAO_Imp  implements Student_DAO{
	...
	public List<Student> getStudents() {
		...
		Query<Student> query=currentSession.createQuery("from Student where status=: status", Student.class);
	...
	public boolean deleteStudent(Student student) {
		...
		try {
			Session currentSession = sessionFactory.getCurrentSession();
			Query query=currentSession.createQuery("UPDATE Student set status=:status WHERE student_id=:student_id");
